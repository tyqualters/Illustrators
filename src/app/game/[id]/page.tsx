...
export default function GameRoomPage() {
  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      ...
      socket.on('game:turnStarted', (turnData: TurnState) => {
        setGameStarted(true);
      ...
      socket.on('game:roundEnded', (/*roundData: RoundEndedData*/) => {
          ...
          <div className="w-3/5">
            {gameEnded ? (
              ...
              <>
                ...
                {isDrawer && !wordConfirmed && currentTurn?.wordOptions?.length && currentTurn.wordOptions.length > 0 && (
                ...
                <div className="w-full h-[400px] border mb-6 relative">
                  <GameCanvas className="w-full h-full" socket={socketRef.current ?? undefined} isDrawing={canDraw} />
                  {!canDraw && (
            ...
            <div className="flex-grow overflow-y-auto text-left mb-2 border p-1 h-[300px]">
              <ul>
                {messages.map((msg, idx) => {
                  const isCorrect =
                    Array.isArray(correctGuessers) && correctGuessers.includes(msg.playerId);
                  const shouldShow =
                    isDrawer ||
                    ((Array.isArray(correctGuessers) &&
                      typeof socketRef.current?.id === 'string' &&
                      correctGuessers.includes(socketRef.current.id)) === isCorrect);
                  return shouldShow ? (
                    <li key={idx}>
                      <strong>{getName(msg.playerId)}</strong>: {msg.text}
                    </li>
                  ) : null;
                })}
              </ul>
